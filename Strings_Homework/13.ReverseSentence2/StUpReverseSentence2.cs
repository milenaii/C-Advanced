...
namespace _13.ReverseSentence2
{
    class StUpReverseSentence2
    {
        ...
        static void Main()
        {
            ...
            PrefillPunctuation(separatedSentence, result);  //до тук е ок
        ...
        // тук вече ми е по-мътно, тр да го разгл по-подробно
        ...
        private static void PrefillPunctuation(string[] separatedSentence, string[] result)
        {
            ...
            {
                if (punctuation.Contains(separatedSentence[i]))
                {
                    result[i] = separatedSentence[i];
                }
            }
        }
        ...
        {
            // слагаме " " около пунктоациите, за да можем да ги разделим
            foreach (var character in punctuation)
            {
                input = input.Replace(character, " " + character + " ");
            }
            string[] separatedSentence = input.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
