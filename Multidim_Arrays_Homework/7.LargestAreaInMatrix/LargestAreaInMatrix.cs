...
namespace Problem7.LargestAreaInMatrix
{
 ...
 //Логика: Ще взимаме всеки елемент на матрицата и ще проверяваме рекурсивно дали
 //има съседен равен на него, ако да увеличаваме текущата поредица и отбелязваме
 //че вече сме проверили този елемент по някакъв начин.След като имаме дължина
 //на текуща поредица ще я сравняваме с най-голямата дължина намерена до
 //момента(първоначално тя ще е 0) и ако е по-голяма, най-голямата поредица
 //ще става текущата, тогава ще си отбележим и кой е елемента който я съставя...
        ...
        static void FindLargestAreaLength(int[,] matrix)
        {
            ...
            // ще вземаме поредният елемент, и за всеки такъв ще извикваме метода който
            //ще намира текущата дължина на поредицата
            // след това имайки текущата дължина ще проверим дали тя е най-голяма,
            //ако да си записваме текущият елемент който изгражда поредицата
            ...
            // на края показваме на потребителя колко е дълга най-голямата поредица
            //от съседни равни елементи и кой е този елемент
        ...
        {
            // Aко се намираме извън матрицата или на елемент който вече е бил 
            //проверен за съседност метода се прекратява.Това ще бъде
            // дъното на рекурсията с помощта на която ще намираме всички съседни 
            //равни елементи.
            ...
            if (matrix[row, col] == currentElement)// това първоначално ще сравнява 
                                                   //текущият елемент със себе си, но 
                                                   //ще е от полза при рекурсията тъй
                                                   // като тя ще проверява за елемента
                                                   //отгоре, долу, ляво или дясно 
            {
                // ще го отбелязваме, че е съседен, равен и проверен като го променим
                //на минималната за типа стойност
        ...
        // този метод ще принтира матрицата за да се убеди потребителя във верността
        //на получените данни
